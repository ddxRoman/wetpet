...
<title>Кабинет — {{ $doctor->name ?? '' }} — Зверозор</title>
        ...
        <div class="col-lg-3">
            <div class="card border-0 shadow-sm rounded-3 sticky-top p-0" style="top:16px;">
                ...
                <div class="text-center p-4 border-bottom">
                    @php $photoField = in_array('doctor', ['doctor','specialist']) ? 'photo' : 'logo'; @endphp
                    @if(!empty($entity->{$photoField}))
                        <img src="{{ Storage::url($entity->{$photoField}) }}"
                             class="{{ in_array('doctor', ['doctor','specialist']) ? 'rounded-circle' : 'rounded-3' }} mb-2"
                    ...
                    @else
                        <div class="rounded-3 bg-primary bg-opacity-10 d-flex align-items-center justify-content-center mb-2 mx-auto"
                            ...
                            {{ ['clinic'=>'🏥','organization'=>'🏢','doctor'=>'👨‍⚕️','specialist'=>'🩺']['doctor'] }}
                        </div>
                    @endif
                ...
                <nav class="p-2">
                    ...
                    @foreach(['info' => ['icon'=>'📋','label'=>'Основная информация'], 'photos' => ['icon'=>'📷','label'=>'Фотографии'], 'services' => ['icon'=>'💊','label'=>'Услуги и цены'], 'seo' => ['icon'=>'🔍','label'=>'SEO']] as $key => $tab)
                        <a href="?tab={{ $key }}"
                    ...
                    @if(!empty($entity->slug))
                        <a href="{{ url('doctors/' . $entity->slug) }}" target="_blank"
