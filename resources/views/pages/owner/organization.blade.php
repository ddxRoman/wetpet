...
<div class="container my-4" style="max-width: 1100px;">
    ...
    {{-- Переключатель между организациями пользователя (если их больше одной) --}}
    ...
    @if(!$isConfirmed)
        ...
        {{-- ═══════ ОРГАНИЗАЦИЯ НА ПРОВЕРКЕ — форма документов ═══════ --}}
        <div class="row g-4">
            <div class="col-lg-3">
                <div class="card border-0 shadow-sm rounded-3 p-0">
                    <div class="text-center p-4 border-bottom">
                        @if(!empty($entity->logo))
                            <img src="{{ Storage::url($entity->logo) }}"
                                 class="rounded-3 mb-2"
                                 style="width:80px;height:80px;object-fit:cover;">
                        @else
                            <div class="rounded-3 bg-warning bg-opacity-10 d-flex align-items-center justify-content-center mb-2 mx-auto"
                                 style="width:80px;height:80px;font-size:32px;">🏢</div>
                        @endif
                        <div class="fw-bold text-dark" style="font-size:14px;">{{ $entity->name ?? '' }}</div>
                        <span class="badge bg-warning text-dark mt-2" style="font-size:11px;">⏳ На проверке</span>
                    </div>
                </div>
            </div>
    ...
    @else
        ...
        {{-- ═══════ ОРГАНИЗАЦИЯ ПОДТВЕРЖДЕНА — обычный кабинет ═══════ --}}
        <div class="row g-4">
