    ...
    <div class="ms-md-3 mt-3 mt-md-0">
        @auth
            @php
                ...
                // Есть ли активная (не отклонённая) заявка на ДРУГОГО специалиста/доктора
                $hasOtherClaim =
            ...
            @elseif($alreadyOwner && !$alreadyOwner->is_confirmed && !$myRejected)
                {{-- Заявка на эту карточку уже подана и на рассмотрении --}}
            ...
            @elseif((!$alreadyOwner || $canReapply) && !$isAnySpecialistConfirmed && !$hasOtherClaim)
                {{-- Нет заявки или старая отклонена и прошло 7 дней — показываем кнопку --}}
