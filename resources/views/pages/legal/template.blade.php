...
@section('title', ($page->meta_title ?? $page->title) . ' — Зверозор')
...
<div class="container my-5" style="max-width: 900px;">
    ...
    <div class="text-center mb-5">
        <h2 class="legal_h2 fw-bold text-dark mb-2">{{ $page->title }}</h2>
        <p class="text-muted small">{{ $page->subtitle ?? 'Действующая редакция. Территория Российской Федерации' }}</p>
    </div>

    {{-- Основной блок --}}
    <div class="card shadow-sm border-0 p-4 p-md-5 bg-white mb-5" style="border-radius: 16px;">
        
        {{-- Вводная плашка (если заполнена в БД) --}}
        @if($page->intro)
        <div class="p-3 mb-4 bg-light rounded-3 border-start border-primary border-4">
            <div class="text-secondary mb-0" style="line-height: 1.6; font-size: 0.95rem;">
                {!! $page->intro !!}
            </div>
            {{-- Данные компании из конфига подставляются автоматически --}}
            <p class="text-secondary mt-2 mb-0" style="line-height: 1.6; font-size: 0.95rem;">
                Оператором персональных данных является: <b>{{ config('company.fio') }}</b> ИНН: <b>{{ config('company.inn') }}</b>, адрес регистрации: <b>{{ config('company.address') }}</b>.
            </p>
        </div>
        @endif

        {{-- Основной контент из БД --}}
        <div class="legal-content text-secondary" style="font-size: 0.95rem; line-height: 1.7;">
            {!! $page->content !!}
        </div>

        {{-- Финальная плашка безопасности (статичная для всех документов) --}}
