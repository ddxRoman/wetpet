<div class="tab-content" id="reviews" style="display:none;">
    <h2>Ваши отзывы {{ $user->name ?? $user->name }}</h2>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadReviews() {
        try {
            const res = await fetch(`/account/reviews/{{ $user->id }}`, { credentials: 'same-origin' });
            if (!res.ok) {
                const txt = await res.text();
                console.error('getReviews failed', res.status, txt);
    ...
    function renderCard(r) {
        ...
        const receipts = (r.receipt && r.receipt.path)
            ? `<div class="media-group"><strong>Чек:</strong>
                <div class="media-item">
                    <a href="/storage/${r.receipt.path}" target="_blank" class="receipt-link">📄 Чек</a>
                    <button type="button" class="btn-del-receipt" data-receipt-id="${r.receipt.id}">×</button>
                </div>
               </div>`
            : '';
    ...
    reviewsList.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('btn-save')) {
            ...
            // append files
            const photosInput = card.querySelector('.input-photos');
            if (photosInput && photosInput.files.length) {
                Array.from(photosInput.files).forEach(f => formData.append('photos[]', f));
            }
            const receiptsInput = card.querySelector('.input-receipts');
            if (receiptsInput && receiptsInput.files.length) {
                Array.from(receiptsInput.files).forEach(f => formData.append('receipts[]', f));
            }
